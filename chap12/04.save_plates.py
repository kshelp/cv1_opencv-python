...
from plate_preprocess_save import *        # 전처리 및 후보 영역 검출 함수
from plate_candidate import *         # 후보 영역 개선 및 후보 영상 생성 함수

nsample = 70
for car_no in range(nsample):
    try:
        print(car_no)
        #car_no = int(input("자동차 영상 번호 (0~69): "))
        ...
        for i, idx in enumerate(correct):
            ...
            resized_image = cv2.resize(candidate_imgs[idx], (144, 28))
            cv2.imwrite("images/plate/%03d.png" %car_no, resized_image)
